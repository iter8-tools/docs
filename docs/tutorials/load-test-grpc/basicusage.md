...
Refer to the [chart's `values.yaml` file](../../user-guide/topics/default-values.md) for additional parameters related to the load profile such as `duration`, `maxDuration`, `connectTimeout`, and `keepalive`.
